...
const ShowSingleCard = ({ show }) => {
  ...
  function generosCoincidentes(showGeneros, generoSeleccionados) {
    return generoSeleccionados.filter((gen) => showGeneros.includes(gen))
      .length;
  }
  useEffect(() => {
    ...
    if (resultFilter.length < 2) {
      const resultFilter2genres = shows.filter(
        (serie) => generosCoincidentes(serie.genres, show.genres) >= 2
      );
